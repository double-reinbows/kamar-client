...
public class GuestHousekeepingActivity extends AppCompatActivity implements TextWatcher {
...
/*
    ...
    protected void createList(List<String> hkOptions, ListView view) {
    ...
    public void afterTextChanged(Editable s) {
        Log.d(GuestHousekeepingActivity.class.getCanonicalName(), "Validating form");
        validate();
    }
    ...
    private void validate() {
        ...
        if (editTransportDestination.getText().toString().trim().equalsIgnoreCase("")) {
            Log.d(GuestHousekeepingActivity.class.getCanonicalName(), "Transport field empty");
            invalid = true;
        ...
        try {
            ...
            if (passengers.equalsIgnoreCase("")) {
                Log.d(GuestHousekeepingActivity.class.getCanonicalName(), "Passenger field empty");
                invalid = true;
                editTransportPassengers.setError(getString(R.string.required));
            } else {
                if (Integer.parseInt(passengers) <= 0) {
                    Log.d(GuestHousekeepingActivity.class.getCanonicalName(), "Passenger number not greater than 0");
                    invalid = true;
                    editTransportPassengers.setError(getString(R.string.required));
                }
            }
        } catch (NumberFormatException e) {
            Log.d(GuestHousekeepingActivity.class.getCanonicalName(), "Number format exception");
            invalid = true;
            editTransportPassengers.setError(getString(R.string.required));
        }
        Log.d(GuestHousekeepingActivity.class.getCanonicalName(), "Setting submit button to " + !invalid);
     ...
     */
    private void sendHousekeepingRequest(String destination, int passengers, String departureIn,
                                      String message) {
        Transport transport = new Transport(message, passengers,
                departureIn, destination);
        Log.d(GuestHousekeepingActivity.class.getCanonicalName(), "sendTransportRequest() to " +
        ...
        if (!guestId.equals("none") && !roomNumber.equals("none")) {
            PermintaanServer.getInstance(this.getBaseContext()).createPermintaan(new Permintaan(
                ...
                @Override public void onCompleted() {
                    Log.d(GuestHousekeepingActivity.class.getCanonicalName(), "createPermintaan() On completed");
                    finish();
                }
                @Override public void onError(Throwable e) {
                    Log.d(GuestHousekeepingActivity.class.getCanonicalName(), "createPermintaan() On error");
                    e.printStackTrace();
                    Toast.makeText(
                            GuestHousekeepingActivity.this,
                ...
                @Override public void onNext(Permintaan permintaan) {
                    Log.d(GuestHousekeepingActivity.class.getCanonicalName(), "createPermintaan() On next" + permintaan);
                    if (permintaan != null) {
                        Toast.makeText(
                                GuestHousekeepingActivity.this,
            ...
            Log.d(GuestHousekeepingActivity.class.getCanonicalName(), "Guest ID is none");
            Toast.makeText(
                    GuestHousekeepingActivity.this,
                    getString(R.string.something_went_wrong),
                    Toast.LENGTH_LONG
